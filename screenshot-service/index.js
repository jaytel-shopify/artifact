...
app.get('/', async (req, res) => {
  ...
  let browser;
  try {
    console.log(`Taking screenshot of: ${url} (${width}x${height}), auth: ${useAuth}`);
    
    // Get IAP token for Shopify domains
    let iapToken = null;
    if (useAuth && (url.includes('.shopify.io') || url.includes('.shopify.com'))) {
    ...
    if (iapToken) {
      ...
      page.on('request', (request) => {
        ...
        if (requestUrl.includes('.shopify.io') || requestUrl.includes('.shopify.com')) {
          const headers = {
            ...request.headers(),
    ...
    const onAuthPage = currentUrl.includes('accounts.google.com') || currentUrl.includes('signin');
    if (onAuthPage) {
    ...
    console.log(`Screenshot completed for: ${url} (${screenshot.length} bytes), auth page: ${onAuthPage}`);
